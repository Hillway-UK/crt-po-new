...
export default function Dashboard() {
  ...
  // CEO Dashboard - CEO-specific approval focused
  if (user.role === 'CEO') {
...
function CEODashboard({ user }: { user: any }) {
  const { data: stats } = useQuery({
    queryKey: ['ceo-dashboard-stats', user.organisation_id],
    queryFn: async () => {
      // Fetch pending CEO approval POs
      ...
      // Fetch pending invoices (CEO sees same as MD for invoices)
      ...
      const approvedValueToday = approvedData?.reduce((sum, po) => sum + (po.amount_inc_vat || 0), 0) || 0;

      // Fetch invoices approved today
      const { count: invoicesApprovedToday } = await supabase
        .from('invoices')
        .select('*', { count: 'exact', head: true })
        .eq('organisation_id', user.organisation_id)
      ...
      return {
        ...
        approved_value_today: approvedValueToday,
        invoices_approved_today: invoicesApprovedToday || 0,
      };
    },
  });
    ...
    <MainLayout title="Dashboard">
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <Card className="border-l-4 border-l-amber-400">
            <CardHeader>
              <CardTitle>Pending CEO Approvals</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <div className="flex justify-between items-center">
                <span className="text-sm">High-Value Purchase Orders</span>
                <span className="text-2xl font-bold">{stats?.pending_pos || 0}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm">Invoices</span>
